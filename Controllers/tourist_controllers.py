...
def create_tourist(new_tourist: dict):
...
def get_all_tourists():
    tourists = Tourist.get_all_tourist_data(session)
    return tourists

def get_tourist_by_id(id):
    tourist = Tourist.get_tourist_data(session, id)
    if tourist:
        return tourist
    else:
        raise HTTPException(status_code=404, detail="Tourist not found")
    
def update_tourist(data):
    tourist = Tourist.update_tourist_data(session, data)
    if tourist:
        return tourist
    else:
        raise HTTPException(status_code=404, detail="Tourist not found")
    
def delete_tourist(id_data):
    id = id_data['tourist_id']
    tourist = Tourist.delete_tourist_data(session, id)
    if tourist:
        return tourist
    else:
        raise HTTPException(status_code=404, detail="Tourist not found")
